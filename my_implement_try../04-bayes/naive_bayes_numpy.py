...
def Train(trainset, train_labels):
    ...
    for i in range(class_num): # 在确定Y=c_k的条件概率
        for j in range(feature_len): # 特征取值
            ...
            # 计算0，1像素点对应的条件概率
            probalility_0 = (float(pix_0)/float(pix_0+pix_1)) 
            probalility_1 = (float(pix_1)/float(pix_0+pix_1)) 
...
def calculate_probability_(img):
    probability = prior_probability.copy()
    #print("----prior_probability: ", prior_probability)
    # (10, 784, 2)
    #print("------probability: ", probability.shape)
    for i in range(len(img)):
        #print("--------probability.shape: ", probability.shape)
        #print("------conditional_probability[:, i, img[i]]: ", conditional_probability[:, i, img[i]].shape)
        probability = probability * conditional_probability[:, i, img[i]]
...
def Predict(testset, prior_probability, conditional_probability):
    ...
    for img in testset:
        ...
        '''
        pro = calculate_probability_(img)
        print("++++++pro: ", pro)  
        max_ = max(pro)
        #print("++++++max_: ", max_)
        max_label = np.where(pro==np.max(pro, axis=0))
        print("++++++max_label: ", max_label)  
        np.squeeze(max_label)
        print("++++++max_label: ", max_label)  
        predict.append(max_label[0])
